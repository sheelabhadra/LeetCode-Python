...
"""SOLUTION:
Solve the equation, candyA + x - y = candyB - x + y. For searching in B convert it into a set. 

"""

class Solution:
    def fairCandySwap(self, A: List[int], B: List[int]) -> List[int]:
        candyA, candyB = sum(A), sum(B)
        B = set(B)
        for c in A:
            if c + (candyB - candyA)//2 in B:
                return [c, c + (candyB - candyA)//2]
